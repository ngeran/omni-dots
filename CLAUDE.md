...
1. **`modules/stylix.nix`** — [Stylix](https://stylix.danth.me/) generates a base16 palette **at build time** from `wallpaper.jpg`, in **palette-only mode** (`autoEnable = false`) so it does NOT theme apps itself (that would fight Quickshell's live theming). This is the **cold-boot seed** of a *hybrid*: **matugen** (`modules/matugen.nix`, from `github:InioX/matugen`) is the **runtime** generator Quickshell runs on the live wallpaper for instant palettes with **no rebuild**. nixpkgs `matugen` 4.0.0's `image` subcommand is broken — hence the flake input. `qs-apply-wallpaper` (here) is the rebuild *fallback*; matugen is primary.
