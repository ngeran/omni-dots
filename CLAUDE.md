...
- System modules (imported in `hosts/desktop/default.nix`): `audio.nix` (PipeWire+WirePlumber), `bluetooth.nix`, `nvidiagpu-compute.nix` (NVIDIA driver/CUDA/Ollama), `virtualization.nix` (libvirt+docker), `greetd.nix` (tuigreet → `start-hyprland`), `file-manager.nix` (Thunar), `stylix.nix`, `fonts.nix`, `apps/desktop-apps.nix`, `apps/dev-tools.nix` (which also imports `apps/ai-tools/` — one file per AI/agent-dev CLI: jq, yq, uv, tmux, git-delta, docker-compose, websocat — so both hosts get them).
...
1. **`modules/stylix.nix`** — [Stylix](https://stylix.danth.me/) generates a base16 palette **at build time** from `wallpaper.jpg`, in **palette-only mode** (`autoEnable = false`) so it does NOT theme apps itself (that would fight Quickshell's live theming). This is the **cold-boot seed** of a *hybrid*: **matugen** (`modules/matugen.nix`, taken from `nixpkgs-unstable` — the channel's 4.0.0 has a broken `image` subcommand; unstable ships working 4.2.0) is the **runtime** generator Quickshell runs on the live wallpaper for instant palettes with **no rebuild**. `qs-apply-wallpaper` (here) is the rebuild *fallback*; matugen is primary.
...
3. **`~/.cache/theme/colors.json`** — the **live, writable** theme channel. The Quickshell settings process writes it **atomically** (tmp + rename) on every theme change; the bar watches it via FileView. Seeded once (only if absent) by activation scripts in `modules/apps/essentials.nix` (`seedThemeColors`, plus `seedHyprlockColors` for the lock screen's `~/.config/hypr/quickshell-colors.conf`).
...
**Changing the wallpaper:** matugen generates a new palette from the live wallpaper **instantly at runtime** (no rebuild) — the primary path (dashboard toggle "Auto-theme from wallpaper"). The legacy rebuild path — `qs-apply-wallpaper <image>` (system package from `modules/stylix.nix`) copies the image into the flake tree as `wallpaper.jpg` and rebuilds so Stylix regenerates the seed — is a fallback (`fallbackToRebuild`, off by default). When that fallback runs, the dashboard invokes it via `pkexec` under a **passwordless polkit rule** (for `wheel`, in `modules/stylix.nix`) — no GUI prompt and no sudoers entry; the rule itself needs one manual `sudo nixos-rebuild switch` to land. Paths inside are hardcoded to `/home/nikos` because pkexec runs as root.
